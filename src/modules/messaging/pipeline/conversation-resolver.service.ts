  ...
  async resolve(
    ...
    const openConversation = await this.prisma.conversation.findFirst({
      where: {
        ...
        channelId,
        contactId,
        status: { in: [ConversationStatus.PENDING, ConversationStatus.OPEN, ConversationStatus.BOT, ConversationStatus.WAITING] },
      },
    ...
    this.logger.log(`New conversation created: ${conversation.id} (protocol: ${protocol})`);
